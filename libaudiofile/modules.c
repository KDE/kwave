...
extern _AFmodule _AFint2rebufferv2f, _AFint2rebufferf2v;
	...
	if (compunit->needsRebuffer)
	{
		...
		if (h->access == _AF_WRITE_ACCESS)
			track->ms.filemod_rebufferinst =
				_AFinitint2rebufferv2f(chunkframes*track->f.channelCount,
					compunit->multiple_of);
		else
			track->ms.filemod_rebufferinst =
				_AFinitint2rebufferf2v(chunkframes*track->f.channelCount,
					compunit->multiple_of);
...
*/
static status addfilereadmods (current_state *current, _Track *track, AFfilehandle h)
{
...
/*
	...
	for a given AFfilehandle.
*/
static status addfilewritemods (current_state *current, _Track *track, AFfilehandle h)
{
	assert(track->ms.filemodinst.valid);
...
*/
static bool useAP (double inrate, double outrate, double *inratep, double *outratep)
{
	bool instandard =
