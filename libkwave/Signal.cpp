...
//**********************************************************
Signal::Signal (int *signal, int numsamples, int rate)
...
//**********************************************************
Signal::Signal (int numsamples, int rate)
// constructor for creating a "silence"-sample
{
    this->rate = rate;
    this->length = 0;
	...
	printf("Signal::Signal(): out of Memory !\r\n");
    }
}
...
//*********************************************************
void Signal::lockRead () {
    locked++;
}
//*********************************************************
void Signal::unlockRead () {
    locked--;
}
//*********************************************************
void Signal::lockWrite () {
    locked = -1;
}
//*********************************************************
void Signal::unlockWrite () {
    locked = 0;
}
//////*********************************************************
////bool Signal::command (TimeOperation *operation) {
////    const char *command = operation->getCommand();
////
////    // debug("Signal:command(%s)",command);
////
////    len = rmarker - lmarker;
////    begin = lmarker;
////
...
////
////    while ((globals.timeplugins().at(cnt)) && (!done)) {
////	if (strcmp(parse.getCommand(),
////		globals.timeplugins().at(cnt)->getName()) == 0)
////	{
////	    void *handle = dlopen(globals.timeplugins().at(cnt)->getFileName(),
////		RTLD_NOW);
////	    if (handle) {
