...
{
    return new PulseDialog(operation->getRate(), 
                           operation->getLength(), operation->isModal());
}

//**********************************************************
PulseDialog::PulseDialog(int rate, int time, bool modal)
    :Dialog(modal) 
{
    ...
    char buf[512];
    snprintf(buf, sizeof(buf), "curve (linear,100, %f)", 
	((double)rate) / 440);
    times = new Curve (buf);
...
void PulseDialog::resizeEvent(QResizeEvent *) 
{
    ...
    pulselabel->setGeometry(width*5 / 10 + 2, height - bsize*3, 
                            width*3 / 10-2, bsize);
    pulselength->setGeometry(width*8 / 10, height - bsize*3, 
                             width*2 / 10-8, bsize);
