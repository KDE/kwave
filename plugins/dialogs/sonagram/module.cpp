...
{
    return new SonagramDialog(operation->isModal(), 
                              operation->getLength(), operation->getRate());
}
...
void SonagramDialog::setPoints(int points) 
{
	...
	i18n("resulting window size: %s"), 
	(mstotimec(points*10000 / rate)));
...
void SonagramDialog::resizeEvent(QResizeEvent *) 
{
    ...
    pointslider->setGeometry(width() / 2, bsize*5 / 2, 
                             width() / 2 - 8, bsize);
