 ...
 if (msg[processid]<=0)
  {
   ...
   if (proc==0)
    ...
    else KMsgBox::message (parent,"Info","Can not open /dev/dsp !",2);
    debug ("%d terminated !\n",proc);
    msg[processid]=0;
    exit (0);
   }
	...
	if (sigin->exists())
	 {
	  union
	   ...
	   if (num==sizeof(struct wavheader))
	    {
		if ((strncmp("RIFF",header.riffid,4)==0)&&(strncmp("WAVE",header.wavid,4)==0)&&(strncmp("fmt ",header.fmtid,4)==0))
		 {
				  	...
				  	switch (header.bitspersample)
			  		 {
						...
						break;
						default:
							KMsgBox::message (parent,"Info","Sorry only 8&16 Bits per Sample are supported this time !",2);
						break;	
					 }
					emit sampleChanged();
				 }
...
void	MSignal::load16Bit (QFile *sigin,int offset,int interleave)
{
  ...
  if (sample)
   {
	QDialog		*dialog	=new QDialog (0,"Load Info");
	KProgress	*prog	=new KProgress (dialog,"Load Progress");

	prog->setGeometry (0,0,100,20);
	dialog->show();
