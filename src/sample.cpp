...
KWaveMenuItem fx_menus[]=
{
  ...
  {FADEIN         ,"Fade &in"           ,KITEM ,-1   ,Key_I},
  {FADEOUT        ,"Fade &out"          ,KITEM ,-1   ,Key_O},
...
KWaveMenuItem sample_edit_menus[]=
{
...
void MSignal::appendMenus ()
{
  if (manage)
      ...
      if (manage->addNumberedMenu ("FilterPresets"))
	{
      ...
      manage->appendMenus (fx_menus);
      manage->appendMenus (calculate_menus);
      manage->appendMenus (sample_edit_menus);
    }
}
//**********************************************************
void MSignal::deleteMenus ()
{
  if (manage)
    {
      manage->deleteMenus (calculate_menus);
      manage->deleteMenus (fx_menus);
      manage->deleteMenus (sample_edit_menus);
    }
}
//**********************************************************
void MSignal::getIDs ()
  ...
  // doing a port to another system...
{
  ...
  if (manage) id=manage->translateId (fx_menus,id);
  if (manage) id=manage->translateId (calculate_menus,id);
  if (manage) id=manage->translateId (sample_edit_menus,id);
  ...
  //does a little bit to much, but nothing should hurt the performance
{
  ...
  emit selectedTimeInfo ((lmarker-rmarker)*10000/rate);
  emit lengthInfo (length);
  emit timeInfo ((length)*10000/rate);
  ...
  //this one sets the internal markers, most operations use
{
  lmarker=l;
  rmarker=r;
  emit selectedTimeInfo((int)((r-l)*10000/rate));
